...
- **Logout** (per [docs/APP_LOGOUT_GUIDE.md](docs/APP_LOGOUT_GUIDE.md), follow it exactly): app-level logout = clear localStorage token + clear the shared `sso_token` cookie + best-effort audit POST `POST /api/auth/logout` (401 also counts as success). The cookie is non-HttpOnly, `secure`, `path=/`, `domain=.smartbid.site` — **deleting it requires an exact attribute match** (`path=/; domain=.smartbid.site`); omitting `domain` or using the site host silently fails (guide §3 pitfall — grid hit it). `deleteSsoCookie()` writes the two canonical variants and warns if the cookie survives. A sessionStorage suppress flag additionally blocks `initSSO`'s cookie fallback in the same tab after logout, until an explicit login returns a fresh URL token. The audit endpoint does **not** revoke JWTs (stateless, by design — grid's server proxies the call to dodge CORS preflight). Family-wide logout (`GET /logout?redirect=` + token-version revocation, 4A R1/R2) is **pending on the 4A side — do not wire it until 4A ships it** (see [docs/4A_LOGOUT_REQUIREMENTS.md](docs/4A_LOGOUT_REQUIREMENTS.md)).
