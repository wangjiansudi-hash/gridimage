...
- **4A logout reality (measured 2026-09-06, don't re-chase this)**: 4A tokens are stateless JWTs — `POST /api/auth/logout` returns 200 `登出成功` but does **not** revoke anything (the same token still verifies 200 right after; `last_login_at` gets bumped instead). The 4A login page also holds an HttpOnly IdP-session cookie on `auth.smartbid.site` that no sub-app JS can clear, so after a grid logout an explicit re-login bounces straight back with the same token (`client_id` may even be another app's, e.g. `sph`). Grid-side logout is therefore inherently app-local; a real "log out everywhere" needs a 4A-side `GET /logout?redirect=` page (session-cookie clearing + token revocation).
