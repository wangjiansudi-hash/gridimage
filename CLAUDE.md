...
- **Logout** (per [docs/APP_LOGOUT_GUIDE.md](docs/APP_LOGOUT_GUIDE.md), follow it exactly): **two levels**. App-level (`clearToken()`): clear localStorage token + clear the shared `sso_token` cookie + best-effort audit POST `POST /api/auth/logout` (401 also counts as success). The cookie is non-HttpOnly, `secure`, `path=/`, `domain=.smartbid.site` — **deleting it requires an exact attribute match** (`path=/; domain=.smartbid.site`); omitting `domain` or using the site host silently fails (guide §3 pitfall — grid hit it). `deleteSsoCookie()` writes the two canonical variants and warns if the cookie survives; a sessionStorage suppress flag blocks `initSSO`'s cookie fallback in the same tab after logout until an explicit login returns a fresh URL token, and `requireLogin()` force-deletes the cookie before navigating when that flag is set (4A's login-page Auto-SSO bounces back whenever the cookie exists — it never shows the form). Family-wide (`logoutEverywhere()`): 302 to `https://auth.smartbid.site/logout?redirect=<current url>` (4A R1, live since 2026-09-06) — 4A clears the session cookie and revokes all the user's tokens (R2 token-version) then redirects back; wired in the Header logout menu (`btn-logout-app` / `btn-logout-all`). grid's server proxies the audit call to dodge CORS preflight.
- **Server** (`server.js`, systemd unit `grid-quota.service` on the VPS, listens `127.0.0.1:3990`, proxied by Caddy at `/api/*`): `GET /api/quota` (status), `POST /api/quota/consume` (atomically burns 1 unit; 429 `{error:'quota_exceeded'}` when exhausted), and `POST /api/auth/logout` (same-origin proxy that forwards the Bearer token to 4A's audit endpoint and clears the local verify caches). Limits: anonymous **1/day keyed by `Cf-Connecting-Ip`**, authenticated **10/day keyed by 4A numeric user id**; day boundary is Asia/Shanghai midnight; state persisted to `/var/lib/grid-quota/usage.json` (7-day retention).
